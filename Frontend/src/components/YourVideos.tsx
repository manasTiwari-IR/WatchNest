...
const YourVideos: React.FC = () => {
  ...
  const [VideoData, setVideoData] = useState<YourVideosProps[]>([]);

  const fetchYourVideos = async () => {
    try {
      const response = await fetch(`${apiURL}/api/v1/videos/${userdata.username}/${userdata._id}`, {
        method: 'GET',
      ...
      if (response.ok) {
        ...
        // console.log("Fetched videos data: ", data.data.videos[0].videosdata);
        setVideoData(data.data.videos[0].videosdata);
      } else {
  ...
  return (
    <section className='your-playlists-container' >
      <p className='your-playlists-title' >Your Videos</p>
